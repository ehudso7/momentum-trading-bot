...
_REVOKE_OPTION_TOKENS: frozenset[str] = frozenset(
    {
        "-h",
        "--help",
        "--key-hash",
...
def _normalize_revoke_api_key_argv(
    argv: list[str],
) -> list[str]:
    """Protect dash-prefixed API keys from argparse option parsing.

    API keys generated by ``secrets.token_urlsafe`` may legally begin
    with ``-`` or ``--``. When a raw key is supplied as two CLI tokens,

        --api-key -abc...

    argparse can mistake the key for another option and report that
    ``--api-key`` has no value.

    Convert a present value to argparse's unambiguous equals form:

        --api-key=-abc...

    The transformation is in-memory only. The raw key is never logged
    or persisted. When the following token is an actual revoke option,
    leave the arguments unchanged so argparse still reports a genuinely
    missing value.
    """
    normalized: list[str] = []
    index = 0
    ...
    while index < len(argv):
        ...
        if index + 1 >= len(argv):
            # Preserve argparse's normal missing-value error.
            normalized.append(token)
            index += 1
            continue

        candidate = argv[index + 1]
        candidate_option = candidate.split("=", 1)[0]

        if candidate_option in _REVOKE_OPTION_TOKENS:
            # The next token is another real option, not a key value.
            # Preserve argparse's normal missing-value error.
            normalized.append(token)
            index += 1
            continue

        normalized.append(f"--api-key={candidate}")
        index += 2
...
def _revoke_cli(argv: list[str]) -> int:
    normalized_argv = _normalize_revoke_api_key_argv(argv)
    args = _build_revoke_parser().parse_args(normalized_argv)
